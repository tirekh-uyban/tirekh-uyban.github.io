...
>
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				<title>
					<xsl:choose>
						<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">Карта сайта  -</xsl:when>
						<xsl:otherwise>Индекс карты сайта</xsl:otherwise>
					</xsl:choose>
				</title>
		...
		.breadcrumb svg.back {
			...
			<body>
				...
				<xsl:call-template name="Header">
					<xsl:with-param name="title">Карта сайта  -</xsl:with-param>
	...
	<a href="https://tirekh-uyban.ru"><span>Главная</span></a>
	...
	<a href="https://tirekh-uyban.ru"><span>Главная</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="https://tirekh-uyban.ru/sitemap.xml"><span>Индекс карты сайта</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
	...
	<div id="content-head">
		...
		<p>Сгенерировано <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, это XML-карта сайта, предназначенная для использования поисковыми системами, как Google или Яндекс.</p>
		<p>
			Дополнительную информацию об XML-файлах карты сайта можно найти на <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Эта карта сайта содержит						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
					...
					<xsl:otherwise>
						Индекс этой карты сайта содержит						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								карта сайта							</xsl:when>
							<xsl:otherwise>
								карты сайта							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				и была создана 07.02.2024 в 12:29			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">Низкий</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">Средний</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">Высокий</xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<a href="https://tirekh-uyban.ru"><span>Главная</span></a>
		...
		<h2 class="empty-sitemap__title">
			Ой!			<br />
			Записи отсутствуют		</h2>
		<div class="empty-sitemap__buttons">
			<a href="https://tirekh-uyban.ru" class="button">Вернуться на главную</a>
					</div>
